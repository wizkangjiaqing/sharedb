...
/**
 * 客户端获取本地文档版本后的操作时，服务端的操作可能不完整.
 * 需要检查服务端的操作版本是否与客户端的操作版本一致，不一致时，需要返回错误.
 * 否则会导致客户单出现死循环，不断获取操作.
 */
Agent.prototype._checkBeforeSendOps = function(ops, version) {
  if (ops.length === 0) return null;
  if (typeof version !== 'number') return null;
  if (ops[0].v !== version) {
    var message = `Lack some ops for request from ${version}, minimum op in db is ${ops[0].v}`;
    return new ShareDBError(ERROR_CODE.ERR_OP_VERSION_NEWER_THAN_CURRENT_SNAPSHOT, message);
  }
  return null;
};
...
function getReplyErrorObject(err) {
  if (err) logger.warn(err.stack || err, 'reply error');
...
Agent.prototype._fetchOps = function(collection, id, version, callback) {
  ...
  this.backend.getOps(this, collection, id, version, null, function(err, ops) {
    if (err) return callback(err);
    err = agent._checkBeforeSendOps(ops, version);
    if (err) return callback(err);
