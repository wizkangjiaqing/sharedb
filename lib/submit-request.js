  ...
  if (this.shouldWorkInWorker()) {
    ...
    piscina.run({snapshot: this.snapshot, op: this.op}, {name: 'apply'})
      .then(function(result) {
        ...
        logger.info(`piscina apply ${request.op.op.length} ops, total ${total} ms, working ${result.spent} ms`);
        //
...
SubmitRequest.prototype._transformOpCore = function(type, op, appliedOp, callback) {
  ...
  if (this.shouldWorkInWorker()) {
    ...
    piscina
      ...
      .then(function(result) {
        var total = Date.now() - start;
        logger.info(`piscina transform ${request.op.op.length} ops, total ${total} ms, working ${result.spent} ms`);
        //
