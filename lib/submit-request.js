  ...
  if (this.shouldWorkInWorker()) {
    ...
    piscina.run({snapshot: this.snapshot, op: this.op}, {name: 'apply'})
      .then(function(result) {
        ...
        console.info(
          '%s piscina apply %d ops for doc %s:%s, total %d ms, working %d ms, wasted %d ms',
          new Date().toLocaleString(),
          request.op.op.length, request.collection, request.id, total, result.spent, total - result.spent);
        //
  ...
  if (this.shouldWorkInWorker()) {
    ...
    piscina
      ...
      .then(function(result) {
        var total = Date.now() - start;
        console.info(
          '%s piscina transform %d ops for doc %s:%s, total %d ms, working %d ms, wasted %d ms',
          new Date().toLocaleString(),
          request.op.op.length, request.collection, request.id, total, result.spent, total - result.spent);
        //
